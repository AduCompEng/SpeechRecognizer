...
if __name__ == '__main__':
    ...
    while True:
        if not choice:
            choice = input("Type \"voice\" for request commands using voice or \"text\" for request commands by text: ")
        if choice == "text":
            sentence = input("Type: ")
        elif choice == "voice":
            r = sr.Recognizer()
            with sr.Microphone() as source:  # use the default microphone as the audio source
                ...
                print('Wait..')
            try:
                sentence = r.recognize_google(audio, language="tr") #for english  "En"
